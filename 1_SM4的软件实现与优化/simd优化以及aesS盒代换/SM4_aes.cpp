...
void SM4_aes_encrypt(uint8_t* input, uint8_t* enc_result, Keys* round_keys) {
    ...
    for (int i = 0; i < 32; i++) {
        ...
        //L
        tmp[1] = _mm256_xor_si256(_mm256_slli_epi32(tmp[0],  2), _mm256_srli_epi32(tmp[0], 30));
        tmp[2] = _mm256_xor_si256(_mm256_slli_epi32(tmp[0], 10), _mm256_srli_epi32(tmp[0], 22));
        tmp[3] = _mm256_xor_si256(_mm256_slli_epi32(tmp[0], 18), _mm256_srli_epi32(tmp[0], 14));
        tmp[1] = _mm256_xor_si256(tmp[1], tmp[2]);
        tmp[1] = _mm256_xor_si256(tmp[1], tmp[3]);
        tmp[3] = _mm256_xor_si256(_mm256_slli_epi32(tmp[0], 24), _mm256_srli_epi32(tmp[0], 8));
        tmp[1] = _mm256_xor_si256(tmp[1], tmp[3]);
...
void SM4_aes_decrypt(uint8_t* input, uint8_t* dnc_result, Keys* round_keys) {
    ...
    for (int i = 0; i < 32; i++) {
        //异或轮密钥
        tmp[0] = _mm256_xor_si256(_mm256_xor_si256(x[1], x[2]),
            _mm256_xor_si256(x[3], _mm256_set1_epi32(round_keys->rk[31 - i])));
    ...
    x[3] = _mm256_unpackhi_epi64(_mm256_unpackhi_epi32(tmp[3], tmp[2]), _mm256_unpackhi_epi32(tmp[1], tmp[0]));
    //恢复分组并装填
    _mm256_storeu_si256((__m256i*)dnc_result + 0, x[0]);
    _mm256_storeu_si256((__m256i*)dnc_result + 1, x[1]);
    _mm256_storeu_si256((__m256i*)dnc_result + 2, x[2]);
    _mm256_storeu_si256((__m256i*)dnc_result + 3, x[3]);
}