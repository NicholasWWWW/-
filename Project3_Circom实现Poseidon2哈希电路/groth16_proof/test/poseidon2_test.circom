...
//
//  Poseidon2 置换函数（bn128曲线，状态大小t=3）
//
...
//------------------------------------------------------------------------------
// 置换函数的简单应用："压缩函数"接收 2 个域元素作为输入，并输出 1 个域元素。
template Poseidon2Compression() {
  signal input  inp[2];
  signal output out;
...
// 实例化测试电路
template Poseidon2Example () {
    // 隐私输入：原始数据（两个域元素）
    signal input a;
    signal input b;
    
    // 计算实际哈希值
    component hash = Poseidon2Compression();
    hash.inp[0] <== a;
    hash.inp[1] <== b;
    
    // 调试输出
    log("Public hash", hash.out);
}

// 主组件声明：公开哈希值作为验证依据
component main { public [ a ] } = Poseidon2Example();
